...
3. **Handover files** (if the project has a handovers directory):
...
4. **Reference files** (loaded on demand, usually in docs/agents/reference):
...
5. **Agent documentation** (AGENTS.md and auto-loaded context files):
...
6. **Journal entry** (if docs/agents/journal/ exists):
